...
public class OptitrackStreamingClient : MonoBehaviour
{
    ...
    public bool footLeftActive = false;
    public bool footRightActive = false;
    ...
    private GameObject CreateMarkerSphere(GameObject prefab, Vector3 position, string name, int markerId, bool isActive)
    {
        // adjust position relative to calibration
        position.x -= CalibrationLogic.centerPosition.x;
        position.z -= CalibrationLogic.centerPosition.z;

        var sphere = Instantiate(prefab, position, Quaternion.identity);
        sphere.transform.position = position;
        sphere.name = name;
        sphere.transform.Rotate(0, 180, 0);
        ...
        m_latestMarkerSpheres[markerId] = sphere;
        markerNumber++;
    ...
    private void Update()
    {
        // If you have enabled the newly added calibration bool then the following code will run
        // it will draw the markers in the scene and allow you to see where the markers are in real time
        // There is no restriction on how many markers you can have in the scene as you need to see all the markers in this mode
        // It will also limit the markers to be below a certain height and within a certain x range
        if (DrawMarkers && calibration)
        {
            ...
            lock (m_frameDataUpdateLock)
            {
                foreach (KeyValuePair<Int32, OptitrackMarkerState> markerEntry in m_latestMarkerStates)
                {
                    ...
                    {
                        if (markerEntry.Value.Position.y <= 0.6)
                            if (markerEntry.Value.Position.x >= -1 && markerEntry.Value.Position.x <= 1)
                            {
                                {

                                    var sphere = Instantiate(footLeft, markerEntry.Value.Position, Quaternion.identity);
        ...
        // if you are drawing markers and calibration is false then the following code will run
        // this code will only allow for two markers to be drawn in the scene
        // it will also check the distance between the two markers and only update their positions if they are within a certain distance of their last known position
        // this is to prevent the markers from jumping around in the scene if there is noise in the data
        // it will also check if the markers are below a certain height and within a certain x range
        // if the markers go out of this range they will be removed from the scene
        // Marker positions are also adjusted based on the center position calculated during calibration


        else if (DrawMarkers && !calibration)
        {
            if (m_client != null && ConnectionType == ClientConnectionType.Unicast && !m_subscribedToMarkers)
            {
                SubscribeMarkers();
            ...
            lock (m_frameDataUpdateLock)
            {
                ...
                foreach (KeyValuePair<Int32, OptitrackMarkerState> markerEntry in m_latestMarkerStates)
                {
                    ...
                    bool aConditionRan = false;
                    if (m_latestMarkerSpheres.ContainsKey(markerEntry.Key))
                    {
                        markerEntry.Value.Position.x = markerEntry.Value.Position.x - CalibrationLogic.centerPosition.x;
                        markerEntry.Value.Position.z = markerEntry.Value.Position.z - CalibrationLogic.centerPosition.z;


                        if (markerEntry.Key == CalibrationLogic.leftFootMarkerId)
                        {
                            if ((Vector3.Distance(markerEntry.Value.Position, CalibrationLogic.leftFootPosition) < 0.1))
                            {
                                m_latestMarkerSpheres[markerEntry.Key].transform.position = markerEntry.Value.Position;
                                CalibrationLogic.leftFootPosition = markerEntry.Value.Position;

                            }
                        }
                        else if (markerEntry.Key == CalibrationLogic.rightFootMarkerId)
                        {
                            if ((Vector3.Distance(markerEntry.Value.Position, CalibrationLogic.rightFootPosition) < 0.1))
                            {
                                m_latestMarkerSpheres[markerEntry.Key].transform.position = markerEntry.Value.Position;
                                CalibrationLogic.rightFootPosition = markerEntry.Value.Position;
                    ...
                    else
                    {
                        if (markerNumber < 2)
                        {
                            if (markerEntry.Value.Position.y <= 0.6)
                            {
                                if (!footLeftActive)
                                {
                                    if (Vector3.Distance(markerEntry.Value.Position, CalibrationLogic.leftFootPosition) < Vector3.Distance(markerEntry.Value.Position, CalibrationLogic.rightFootPosition) && !(Vector3.Distance(markerEntry.Value.Position, CalibrationLogic.leftFootPosition) > 0.1))
                                    {
                                        markerEntry.Value.Position.x = markerEntry.Value.Position.x - CalibrationLogic.centerPosition.x;
                                        markerEntry.Value.Position.z = markerEntry.Value.Position.z - CalibrationLogic.centerPosition.z;

                                        CalibrationLogic.leftFootMarkerId = markerEntry.Key;
                                        var sphere = CreateMarkerSphere(footLeft, markerEntry.Value.Position, markerEntry.Value.Name, markerEntry.Key, markerEntry.Value.IsActive);
                                        CalibrationLogic.leftFootPosition = sphere.transform.position;
                                        CalibrationLogic.leftFootMarkerId = markerEntry.Key;
                                        footLeftActive = true;
                                ...
                                else if (!footRightActive)
                                {

                                    if (Vector3.Distance(markerEntry.Value.Position, CalibrationLogic.rightFootPosition) < Vector3.Distance(markerEntry.Value.Position, CalibrationLogic.leftFootPosition) && !(Vector3.Distance(markerEntry.Value.Position, CalibrationLogic.rightFootPosition) > 0.1))
                                    {

                                        markerEntry.Value.Position.x = markerEntry.Value.Position.x - CalibrationLogic.centerPosition.x;
                                        markerEntry.Value.Position.z = markerEntry.Value.Position.z - CalibrationLogic.centerPosition.z;

                                        CalibrationLogic.rightFootMarkerId = markerEntry.Key;
                                        var sphere = CreateMarkerSphere(footRight, markerEntry.Value.Position, markerEntry.Value.Name, markerEntry.Key, markerEntry.Value.IsActive);
                                        CalibrationLogic.rightFootPosition = sphere.transform.position;
                                        CalibrationLogic.rightFootMarkerId = markerEntry.Key;
                                        footRightActive = true;
                                        aConditionRan = true;
                                    }
                                ...
                                if (!footLeftActive && !aConditionRan)
                                {
                                ...
                                else if (!footRightActive && !aConditionRan)
                                {
                                    markerEntry.Value.Position.x = markerEntry.Value.Position.x - CalibrationLogic.centerPosition.x;
                                    markerEntry.Value.Position.z = markerEntry.Value.Position.z - CalibrationLogic.centerPosition.z;
                                    CalibrationLogic.rightFootMarkerId = markerEntry.Key;
                ...
                foreach (KeyValuePair<Int32, GameObject> markerSphereEntry in m_latestMarkerSpheres)
                {
                    ...
                    // check marker id with the calibration array, and a true false bool
                    // marker all for removal I think??

                    if (CalibrationLogic.Recalibratebool)
                    {
                        markerSphereIdsToDelete.Add(markerSphereEntry.Key);
                    }
                }

                CalibrationLogic.Recalibratebool = false;

                // remove stale spheres // add bounds and make sure to change current marker postition to an area in the bound
                foreach (Int32 markerId in markerSphereIdsToDelete)
                {
                    if (m_latestMarkerSpheres.ContainsKey(markerId))
                    {
                        if (markerId == CalibrationLogic.rightFootMarkerId)
                        {
                            footRightActive = false;

                            markerNumber--;
                        }
                        if (markerId == CalibrationLogic.leftFootMarkerId)
                        {
                            footLeftActive = false;
                            markerNumber--;
                        }
                        Destroy(m_latestMarkerSpheres[markerId]);
